...
{
    Window::Window() {}
    //  : m_window(nullptr), m_renderer(nullptr), m_width(0), m_height(0), width(m_width), height(m_height) {}
    ...
    void Window::Create(const char *title, unsigned int width, unsigned int height)
    {
        ...
        m_window = SDL_CreateWindow(title, m_width, m_height, 0); // SDL_WINDOW_BORDERLESS
        if (m_window == NULL)
        {
